...
Anomaly detection is performed by the `IntelligenceNotificationService`, which evaluates threshold-based alerts across all six intelligence engine domains. When scores, KPIs, or metrics breach predefined thresholds, alerts are generated and notifications are triggered.

## Engine Pattern

Every engine follows the same signature and produces an `EngineResult` with severity classification:
...
Severity levels drive anomaly classification:
- **critical** — Immediate attention required
- **warning** — Degraded state, action recommended
- **normal** — Within expected parameters
- **good** — Performing above baseline
