...
sidebar_label: Intelligence Platform
description: Six deterministic engines, KPI framework, recommendations, trends, scorecards, and provenance tracking for financial health scoring.
---
...
`kpi-framework.ts` computes 50+ KPIs across five categories after running all six engines. Each KPI has:
...
`scorecard.service.ts` generates role-specific scorecard snapshots (`ceo`, `cfo`, `controller`, `treasurer`, `finance-manager`, `board`, `auditor`). Each scorecard includes:
