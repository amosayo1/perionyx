...
The Integration Platform connects Perionyx to external ERP systems through the plugin-based connector architecture. ERP data flows through the standard connector pipeline: validation → lineage tracking → sync engine → domain modules.
...
This plugin architecture allows new connectors to be added without modifying existing integration code. ERP systems connect via the **HTTP Connector** (generic REST) with configurable authentication.
...
| **HTTP** | **Generic REST** | **Connect to any REST API with configurable authentication (used for ERP)** |
| Mock | Testing | Simulated data for development and sandbox environments |

## Canonical Data Model

All external data — including ERP data — is transformed into a canonical format before entering the platform. This decouples external systems from internal domain models. The canonical model includes:
...
The sync engine manages data synchronization lifecycle for ERP data:

1. **Discovery** — Identifies new or changed data in the external ERP system
2. **Fetch** — Retrieves data through the HTTP connector
