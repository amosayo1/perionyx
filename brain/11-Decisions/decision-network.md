...
| 34 | Documentation claims must be verified against running code before they are trusted — "Fixed", "seeded", and "cached" are runtime properties. Verify against the live database and the actual request path; documentation confidence is not evidence. | Phase 28.0 — Enterprise Readiness Survey. Verified 12 docs/security remediation claims: 9 fixed, 2 PARTIAL (webhook HMAC + Plaid JWS written but never wired to any route), 1 still present (API-key → ADMIN). AP seed "succeeded" against a phantom company ID with zero rows — only a live SELECT revealed it. cacheHeaders emitted no-store — 0 of 212 "cached" GETs ever cached. | Permanent |
| 35 | Re-verify every claim after remediation — a remediation phase is complete only when the final state passes typecheck, production build, the full test suite, and live-database queries; the last edit is the one that ships and must be the one verified. Accepted risks must be documented with plans, not left as silent gaps. | Phase 28.1 — Enterprise Readiness Remediation. All 3 Criticals + 4/5 Highs fixed (proxy identity authority, tick fail-closed, roleFromApiKeyScopes, MFA enforcement, webhook permissions, v1 gating). Removed ignoreBuildErrors → typecheck 0 errors. 9 N+1 loops + 9 unbounded queries batched/bounded. /invoices + /audit-trail rebuilt on live data. Readiness 6.6 → 7.8/10. Verification: typecheck 0 errors, build passes (12 GB heap), touched-area tests 50/50, 47 remaining failures all pre-existing environmental, live-DB smoke passed. | Permanent |
...
*Last updated: 2026-08-08 (Phase 28.1)*
