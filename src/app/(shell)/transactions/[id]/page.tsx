...
export default async function TransactionDetailPage({ params }: RouteProps) {
  return withRuntimeContext(await headers(), async (ctx) => {
    ...
    const { transaction, entries } = await getLedgerForTransactionForTenant(ctx.tenant, transactionId);
    const approvalRequirements = await ApprovalWorkflowEngine.getApprovalRequirements(transactionId, ctx.tenant.companyId);
