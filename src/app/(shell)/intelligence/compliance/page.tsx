...
export default async function CompliancePage() {
  ...
  const [score, kpis, recommendations] = await Promise.all([
    prisma.financialScore.findFirst({
      where: { companyId: ctx.companyId, scoreType: "compliance" },
      orderBy: { calculatedAt: "desc" },
    }),
    prisma.kPIValue.findMany({
      where: { companyId: ctx.companyId, kpiKey: { in: COMP_KPIS as unknown as string[] } },
      orderBy: { recordedAt: "desc" },
      distinct: ["kpiKey"],
    }),
    prisma.intelligenceRecommendation.findMany({
      where: { companyId: ctx.companyId, category: "compliance", status: "active" },
