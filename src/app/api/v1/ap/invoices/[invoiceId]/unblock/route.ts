...
import { unblockInvoiceSchema } from "@/lib/validations/ap";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const parsed = unblockInvoiceSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const result = await invoiceService.unblockInvoice({ invoiceId, reason: parsed.data.reason }, commandCtx);
