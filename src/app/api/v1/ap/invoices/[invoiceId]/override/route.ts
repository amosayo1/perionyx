...
import { overrideMatchSchema } from "@/lib/validations/ap";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    await apRequirePermission(tenant, "ap.invoices.match");
    const { invoiceId } = await context.params;

    const body = await request.json();
    const parsed = overrideMatchSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const result = await invoiceService.overrideMatchResult(parsed.data, commandCtx);
