...
import { rejectInvoiceSchema } from "@/lib/validations/ap";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    await apRequirePermission(tenant, "ap.approvals.reject");
    ...
    const result = await invoiceService.rejectInvoice({ invoiceId, reason: parsed.data.reason }, commandCtx);
