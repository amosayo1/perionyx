...
import { escalateInvoiceSchema } from "@/lib/validations/ap";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    await apRequirePermission(tenant, "ap.approvals.escalate");
    const { invoiceId } = await context.params;

    const body = await request.json();
    const parsed = escalateInvoiceSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const result = await invoiceService.escalateInvoice({ invoiceId, reason: parsed.data.reason }, commandCtx);
