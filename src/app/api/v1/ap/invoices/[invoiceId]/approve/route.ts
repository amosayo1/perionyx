...
import { approveInvoiceSchema } from "@/lib/validations/ap";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    await apRequirePermission(tenant, "ap.approvals.approve");
    const { invoiceId } = await context.params;

    const commandCtx = toCommandContext(tenant, correlationId);
    const result = await invoiceService.approveInvoice({ invoiceId }, commandCtx);
