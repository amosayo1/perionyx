...
import { bulkResolveExceptionsSchema } from "@/lib/validations/ap";
...
export async function POST(request: Request) {
  ...
  try {
    await apRequirePermission(ctx.tenant, "ap.exceptions.resolve");
    ...
    const parsed = bulkResolveExceptionsSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const result = await service.bulkResolveExceptions(parsed.data, cmdCtx);

    if (!result.success) {
      return apErrorResponse(new AppError(result.error!.message, result.error!.code, result.error!.statusCode), ctx.correlationId);
    }

    const resolvedIds = parsed.data.exceptionIds;
    const response = NextResponse.json(
      { data: { resolved: resolvedIds.length, ids: resolvedIds } },
      { status: 200 },
    );
