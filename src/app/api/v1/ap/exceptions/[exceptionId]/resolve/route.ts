...
import { resolveExceptionSchema } from "@/lib/validations/ap";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const parsed = resolveExceptionSchema.safeParse({ ...body, exceptionId });
    if (!parsed.success) {
    ...
    const result = await service.resolveException(
      {
        ...
        resolutionAmount: parsed.data.resolutionAmount as never,
      },
      cmdCtx,
    );
