...
import { escalateExceptionSchema } from "@/lib/validations/ap";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    await apRequirePermission(ctx.tenant, "ap.exceptions.manage");
    ...
    const parsed = escalateExceptionSchema.safeParse({ ...body, exceptionId });
    if (!parsed.success) {
    ...
    const result = await service.escalateException(parsed.data, cmdCtx);
