...
import { rejectPaymentProposalSchema } from "@/lib/validations/ap";
...
export async function POST(req: Request, { params }: { params: Promise<{ proposalId: string }> }) {
  ...
  try {
    ...
    const parsed = rejectPaymentProposalSchema.safeParse(body);
    ...
    const response = NextResponse.json({ data: { proposalId, rejected: true } }, { status: 200 });
