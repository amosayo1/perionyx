...
import { approvePaymentProposalSchema } from "@/lib/validations/ap";
...
export async function POST(req: Request, { params }: { params: Promise<{ proposalId: string }> }) {
  ...
  try {
    ...
    const response = NextResponse.json({ data: { proposalId, approved: true } }, { status: 200 });
