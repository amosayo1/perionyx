...
import { reversePaymentSchema } from "@/lib/validations/ap";
...
export async function POST(req: Request, { params }: { params: Promise<{ batchId: string }> }) {
  ...
  try {
    await apRequirePermission(ctx.tenant, "ap.payments.reverse");
    ...
    const response = NextResponse.json({ data: { batchId, reversed: true } }, { status: 200 });
