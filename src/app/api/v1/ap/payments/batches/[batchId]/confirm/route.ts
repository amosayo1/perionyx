...
import { confirmPaymentSchema } from "@/lib/validations/ap";
...
export async function POST(req: Request, { params }: { params: Promise<{ batchId: string }> }) {
  ...
  try {
    ...
    const response = NextResponse.json({ data: { batchId, paymentId: parsed.data.paymentId, confirmed: true, bankReference: parsed.data.bankReference } }, { status: 200 });
