...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    await apRequirePermission(ctx.tenant, "ap.approvals.reject");
    ...
    const result = await service.rejectLevel(
    ...
    const response = NextResponse.json({ data: { id: approvalChainId, status: "REJECTED" } }, { status: 200 });
