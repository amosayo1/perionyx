...
import { recallApprovalSchema } from "@/lib/validations/ap";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    await apRequirePermission(ctx.tenant, "ap.invoices.delete");
    ...
    const parsed = recallApprovalSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const result = await service.recallApproval(
    ...
    const response = NextResponse.json({ data: { id: approvalChainId, status: "RECALLED" } }, { status: 200 });
