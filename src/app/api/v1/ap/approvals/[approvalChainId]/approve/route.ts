...
import { decideApprovalSchema } from "@/lib/validations/ap";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    await apRequirePermission(ctx.tenant, "ap.approvals.approve");
    ...
    const result = await service.approveLevel(
      { approvalRecordId: approvalChainId, decision: parsed.data.decision, comment: parsed.data.comments },
      cmdCtx,
    );
    ...
    const response = NextResponse.json({ data: { id: approvalChainId, status: "APPROVED" } }, { status: 200 });
