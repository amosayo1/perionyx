...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    await apRequirePermission(ctx.tenant, "ap.approvals.delegate");
    ...
    const parsed = delegateApprovalSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const result = await service.delegateApproval(
      { approvalRecordId: approvalChainId, delegatedTo: parsed.data.delegatedTo, reason: parsed.data.reason },
      cmdCtx,
    );
    ...
    const response = NextResponse.json({ data: { id: approvalChainId, status: "DELEGATED" } }, { status: 200 });
