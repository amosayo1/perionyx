...
import { updateVendorBankDetailsSchema } from "@/lib/validations/ap";
...
export async function PUT(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const parsed = updateVendorBankDetailsSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const result = await vendorService.updateBankDetails({ vendorId, ...parsed.data }, commandCtx);
