...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    await apRequirePermission(tenant, "ap.vendors.approve");
    ...
    if (!parsed.data.reason) {
      return apValidationError([{ path: "reason", message: "Suspension reason is required", code: "required" }], correlationId);
    }

    const commandCtx = toCommandContext(tenant, correlationId);
    const result = await vendorService.suspendVendor({ vendorId, reason: parsed.data.reason }, commandCtx);
