...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (!parsed.data.reason) {
      return apValidationError([{ path: "reason", message: "Rejection reason is required", code: "required" }], correlationId);
    }

    const commandCtx = toCommandContext(tenant, correlationId);
    const result = await vendorService.rejectVendor({ vendorId, reason: parsed.data.reason }, commandCtx);
