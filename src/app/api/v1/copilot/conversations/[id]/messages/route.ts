...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  return withRuntimeContext(request, async (ctx) => {
    try {
      ...
      if (isFirstMessage) {
        const title = await generateTitle(ctx.tenant, body.message);
        await updateConversationTitle(ctx.tenant, id, title);
      }
  
      // Save user message
      const userMsg = await addUserMessage(ctx.tenant, id, body.message.trim());
