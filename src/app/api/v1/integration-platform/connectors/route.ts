...
export async function GET(request: NextRequest) {
  try {
    ...
    const instances = await IntegrationRegistry.listInstances(ctx, { status, category });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const instance = await IntegrationRegistry.createInstance(ctx, { connectorDefId: body.connectorDefId, name: body.name, config: body.config ?? {}, authMethod: body.authMethod as any });
