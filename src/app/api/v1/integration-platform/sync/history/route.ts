...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.status === "completed") {
      sync = await SyncEngineService.completeSync(ctx, body.syncId, body.result);
    } else {
      sync = await SyncEngineService.failSync(ctx, body.syncId, body.result?.error ?? "Unknown error", body.result);
    }
