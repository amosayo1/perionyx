...
export async function GET(request: NextRequest) {
  return withRuntimeContext(request, async (ctx) => {
    try {
      await rbacService.ensurePermission(ctx.tenant.userId, ctx.tenant.companyId, "integration.credential");
      const { searchParams } = new URL(request.url);
      const instanceId = searchParams.get("instanceId");
      if (!instanceId) return NextResponse.json({ error: "instanceId required" }, { status: 400 });
      const credentials = await CredentialManagerService.listCredentials(ctx.tenant, instanceId);
...
export async function POST(request: NextRequest) {
  return withRuntimeContext(request, async (ctx) => {
    try {
      ...
      await CredentialManagerService.storeCredential(ctx.tenant, body.instanceId, body.key, body.value);
...
export async function PUT(request: NextRequest) {
  return withRuntimeContext(request, async (ctx) => {
    try {
      ...
      await CredentialManagerService.rotateCredential(ctx.tenant, body.instanceId, body.key, body.newValue);
...
export async function DELETE(request: NextRequest) {
  return withRuntimeContext(request, async (ctx) => {
    try {
      ...
      await CredentialManagerService.deleteCredential(ctx.tenant, instanceId, key);
