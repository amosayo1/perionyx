...
export async function POST(request: NextRequest) {
  try {
    ...
    await rbacService.ensurePermission(ctx.userId, ctx.companyId, "integration.validation");
    ...
    const issues = await ValidationEngineService.validateImport(ctx, { instanceId: body.instanceId, rows: body.rows });
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [result, summary] = await Promise.all([
      ValidationEngineService.listIssues(ctx, opts),
      ValidationEngineService.getIssueSummary(ctx),
    ]);
