...
export async function GET(request: NextRequest) {
  return withRuntimeContext(request, async (ctx) => {
    try {
      ...
      const items = await FeatureDiscoveryService.getAvailable(ctx.tenant, role ?? "");
  ...
  return withRuntimeContext(request, async (ctx) => {
    try {
      ...
      const flag = body.isEnabled
        ? await FeatureDiscoveryService.enable(ctx.tenant, body.slug)
        : await FeatureDiscoveryService.disable(ctx.tenant, body.slug);
