...
export async function GET(request: Request) {
  return withRuntimeContext(request, async (ctx) => {
    try {
      await rbacService.ensurePermission(ctx.tenant.userId, ctx.tenant.companyId, "reporting.board-pack");
...
export async function POST(request: Request) {
  return withRuntimeContext(request, async (ctx) => {
    try {
      ...
      const boardPack = await generator.generate(ctx.tenant, {
        title: parsed.data.title,
        description: parsed.data.description,
        period: parsed.data.period,
        fiscalYear: parsed.data.fiscalYear,
        audience: parsed.data.audience ?? "board",
        companyIds: parsed.data.companyIds ?? [ctx.tenant.companyId],
      });
