...
export async function GET(request: Request) {
  return withRuntimeContext(request, async (ctx) => {
    try {
      await rbacService.ensurePermission(ctx.tenant.userId, ctx.tenant.companyId, "reporting.view.read");
...
export async function POST(request: Request) {
  return withRuntimeContext(request, async (ctx) => {
    try {
      await rbacService.ensurePermission(ctx.tenant.userId, ctx.tenant.companyId, "reporting.view.write");
      ...
      if (parsed.data.isDefault) {
        await prisma.financialReportSavedView.updateMany({
          where: { companyId: ctx.tenant.companyId, userId: ctx.tenant.userId, reportType: parsed.data.reportType },
