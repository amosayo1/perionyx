...
export async function GET(req: Request) {
  try {
    ...
    const result = await RegulatoryIntelligenceService.getRegulatoryUpdates(ctx, {
      ...filters,
      limit: limit ?? 50,
...
export async function POST(req: Request) {
  try {
    ...
    const result = await RegulatoryIntelligenceService.createUpdate(ctx, parsed.data);
