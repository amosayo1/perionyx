...
export async function GET(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const result = await ComplianceRemediationService.getRemediations(ctx.tenant, {
        ...filters,
        limit: limit ?? 50,
...
export async function POST(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const result = await ComplianceRemediationService.createRemediation(ctx.tenant, parsed.data);
