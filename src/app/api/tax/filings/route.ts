...
import { getFilingsQuerySchema, createFilingSchema } from "@/lib/validations/tax-specialist";
...
export async function GET(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const queryResult = getFilingsQuerySchema.safeParse(Object.fromEntries(url.searchParams));
      ...
      const offset = queryResult.data.page
        ...
        : 0;
      const result = await TaxCalendarService.getFilings(ctx.tenant, {
        ...queryResult.data,
        offset,
      });
...
export async function POST(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const result = await TaxCalendarService.createFiling(ctx.tenant, parsed.data);
