...
import { getReadinessSnapshotsSchema, createReadinessSnapshotSchema } from "@/lib/validations/audit-specialist";
...
export async function GET(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const parsed = getReadinessSnapshotsSchema.safeParse(params);
      if (!parsed.success) {
        return zodErrorResponse(parsed.error, req);
      }
  
      const data = { snapshots: [], filters: parsed.data };
...
export async function POST(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const parsed = createReadinessSnapshotSchema.safeParse(body);
      if (!parsed.success) {
        return zodErrorResponse(parsed.error, req);
      }
