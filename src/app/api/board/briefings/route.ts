...
export async function GET(req: Request) {
  try {
    ...
    const result = await BoardGovernanceFacade.listBriefings(ctx, queryResult.data);
...
export async function POST(req: Request) {
  try {
    const session = await auth();
    const ctx = requireTenantContext(session?.user?.id, session?.user?.activeCompanyId, session?.user?.companyRole);
    await rbacService.ensurePermission(ctx.userId, ctx.companyId, "board.briefings");

    const body = await parseJsonBody<unknown>(req);
    const parsed = generateBriefingSchema.safeParse(body);
    if (!parsed.success) return zodErrorResponse(parsed.error, req);

    const result = await BoardGovernanceFacade.generateBriefing(ctx, parsed.data);
