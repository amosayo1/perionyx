...
export async function GET(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const queryResult = briefingListQuerySchema.safeParse(
        Object.fromEntries(url.searchParams),
      );
      if (!queryResult.success) return zodErrorResponse(queryResult.error, req);
  
      const result = await BoardGovernanceFacade.listBriefings(ctx.tenant, queryResult.data);
...
export async function POST(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      await rbacService.ensurePermission(ctx.tenant.userId, ctx.tenant.companyId, "board.briefings");
  
      const body = await parseJsonBody<unknown>(req);
      const parsed = generateBriefingSchema.safeParse(body);
      if (!parsed.success) return zodErrorResponse(parsed.error, req);
  
      const result = await BoardGovernanceFacade.generateBriefing(ctx.tenant, parsed.data);
