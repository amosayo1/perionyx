...
import { runScenarioSchema } from "@/lib/validations/cfo-advisor";
...
export async function GET(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const scenarioType = url.searchParams.get("scenarioType") as string | undefined;
      const status = url.searchParams.get("status") as string | undefined;
      const page = Number(url.searchParams.get("page") ?? "1");
      const limit = Number(url.searchParams.get("limit") ?? "20");
  
      const result = await CFOAdvisorService.getScenarios(ctx.tenant, {
...
export async function POST(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const scenario = await CFOAdvisorService.runScenario(ctx.tenant, parsed.data);
