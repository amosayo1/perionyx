...
export async function POST(req: Request) {
  try {
    const session = await auth();
    const ctx = requireTenantContext(session?.user?.id, session?.user?.activeCompanyId, session?.user?.companyRole);
    await rbacService.ensurePermission(ctx.userId, ctx.companyId, "admin.access");

    const body = await parseJsonBody<unknown>(req);
    const parsed = createRecommendationSchema.safeParse(body);
    if (!parsed.success) return zodErrorResponse(parsed.error, req);

    const recommendation = await CFOAdvisorService.createRecommendation(ctx, parsed.data);
