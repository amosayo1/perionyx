...
import { getScenariosQuerySchema, createScenarioSchema } from "@/lib/validations/fpa-specialist";
...
export async function GET(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const queryResult = getScenariosQuerySchema.safeParse(
      ...
      const offset = queryResult.data.page
        ...
        : 0;
      const result = await ScenarioModelingService.getScenarios(ctx.tenant, {
        ...queryResult.data,
        offset,
      });
...
export async function POST(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const result = await ScenarioModelingService.createScenario(ctx.tenant, parsed.data);
