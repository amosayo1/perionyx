  ...
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const offset = queryResult.data.page
        ...
        : 0;
      const result = await ExecutiveSupportService.getRecommendations(ctx.tenant, {
        ...queryResult.data,
        offset,
      });
...
export async function POST(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const result = await ExecutiveSupportService.createRecommendation(ctx.tenant, parsed.data);
