...
import { getBudgetsQuerySchema, createBudgetSchema } from "@/lib/validations/fpa-specialist";
...
export async function GET(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const queryResult = getBudgetsQuerySchema.safeParse(
      ...
      const offset = queryResult.data.page
        ...
        : 0;
      const result = await BudgetService.getBudgets(ctx.tenant, {
        ...queryResult.data,
        offset,
      });
...
export async function POST(req: Request) {
  return withRuntimeContext(req, async (ctx) => {
    try {
      ...
      const result = await BudgetService.createBudget(ctx.tenant, parsed.data);
