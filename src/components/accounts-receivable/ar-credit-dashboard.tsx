    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
      <div style={{ background: "#1a1a24", borderRadius: 8, padding: 20 }}>
          ...
          return (
            <motion.div key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} style={{ marginBottom: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                ...
                <span style={{ color: pct > 80 ? "#ef4444" : "#d4af37", fontFamily: "ui-monospace, monospace" }}>{pct.toFixed(0)}%</span>
              </div>
        ...
        <div style={{ marginTop: 16, padding: "12px 0", borderTop: "1px solid #2a2a4a", display: "flex", justifyContent: "space-between", fontSize: 13 }}>
          ...
          <span style={{ color: "#d4af37", fontFamily: "ui-monospace, monospace" }}>${totalExposure.toLocaleString()} / ${totalLimit.toLocaleString()}</span>
        </div>
      </div>
      <div style={{ background: "#1a1a24", borderRadius: 8, padding: 20 }}>
