  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
      {Object.entries(groups).map(([status, items]) => (
        <div key={status} style={{ background: "#1a1a24", borderRadius: 8, padding: 16 }}>
          ...
          {items.map((inv, i) => (
            <motion.div key={inv.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
              ...
              <div style={{ fontSize: 16, fontWeight: 700, color: "#d4af37", fontFamily: "ui-monospace, monospace", marginBottom: 6 }}>${inv.amountDue.toLocaleString()}</div>
