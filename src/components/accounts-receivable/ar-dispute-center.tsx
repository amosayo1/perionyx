    ...
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 12, marginBottom: 20 }}>
        {["open", "investigating", "resolved", "rejected"].map(s => {
          ...
          return (
            <div key={s} style={{ background: "#1a1a2e", borderRadius: 8, padding: 16, textAlign: "center", borderTop: `3px solid ${statusColors[s]}` }}>
      ...
      {disputes.map((d, i) => (
        <motion.div key={d.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
          style={{ background: "#1a1a2e", borderRadius: 8, padding: 16, marginBottom: 8, borderLeft: `3px solid ${statusColors[d.status]}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
            ...
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#d4a843", fontFamily: "ui-monospace, monospace" }}>${d.amount.toLocaleString()}</div>
