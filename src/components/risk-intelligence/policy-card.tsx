...
import type { PolicyMetric } from "./types";
...
export function PolicyCard({ metric }: { metric: PolicyMetric }) {
  ...
  return (
    <div className={cn("rounded-xl border border-white/[0.06] border-l-2 bg-zinc-900/40 p-3 transition-all hover:bg-zinc-900/60", statusColors[metric.status])}>
      ...
      <p className="text-[11px] text-zinc-500 leading-relaxed mt-1">{metric.description}</p>
    </div>
  );
}
