...
import {
  ArrowLeft, Save, Play, Undo2, Redo2, Workflow,
  Plus, Eye, AlertCircle, Sparkles, Settings,
...
import { NODE_WIDTH, NODE_HEIGHT, calculateCanvasSize } from "./utils/layout";
...
export function WorkflowDesigner({ definitionId }: Props) {
  ...
  const debouncedIsDirty = useDebounce(canvas.isDirty, 100);
  ...
  const canvasSize = useMemo(() => calculateCanvasSize(canvas.steps), [canvas.steps]);
  ...
  const handleZoomToFit = useCallback(() => {
    if (typeof window !== "undefined") {
      ...
      if (viewport) {
        ...
        zoom.zoomToFit(canvasSize.width, canvasSize.height, rect.width, rect.height);
      }
    }
  }, [canvasSize, zoom]);
    ...
    <div className="flex h-[calc(100vh-100px)] flex-col">
      ...
      <header className="flex items-center justify-between border-b border-white/[0.06] bg-zinc-950/80 px-4 py-2.5">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Zoom controls */}
          <Button variant="ghost" size="icon" className="h-7 w-7 text-zinc-500" onClick={zoom.zoomOut} title="Zoom Out">
            <ZoomOut className="h-3.5 w-3.5" />
          </Button>
          <span className="w-10 text-center text-[11px] text-zinc-500 tabular-nums">{Math.round(zoom.scale * 100)}%</span>
          <Button variant="ghost" size="icon" className="h-7 w-7 text-zinc-500" onClick={zoom.zoomIn} title="Zoom In">
            <ZoomIn className="h-3.5 w-3.5" />
          </Button>
          <Button variant="ghost" size="icon" className="h-7 w-7 text-zinc-500" onClick={handleZoomToFit} title="Zoom to Fit">
          ...
          <Button variant="outline" size="sm" className="gap-1.5 text-xs" onClick={handleSave} disabled={isSaving}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div
          ...
          className="flex-1 overflow-hidden bg-[radial-gradient(ellipse_at_center,rgba(212,175,55,0.02)_0%,transparent_70%)]"
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex flex-col items-center pt-5">
                              ...
                              {!isLast && (
                                <div className="h-3 w-0.5 bg-gradient-to-b from-[#d4af37]/30 to-transparent" />
                              )}
                            </div>
