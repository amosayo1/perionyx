...
import type { FCAlertsPanelProps } from "./fc-types"
...
const typeColors: Record<string, string> = { close: "#8b5cf6", reconciliation: "#06b6d4", journal: "#3b82f6", approval: "#22c55e", task: "#eab308", variance: "#f97316", compliance: "#ef4444" }

export function FCAlertsPanel({ alerts }: FCAlertsPanelProps) {
  ...
  return (
    <div>
      ...
      <AnimatePresence>
        {filtered.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ color: "#64748b", fontSize: 13, textAlign: "center", padding: 40 }}>No active alerts</motion.div>
        ) : (
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <span style={{ background: typeColors[alert.type] + "22", color: typeColors[alert.type], fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 10, textTransform: "uppercase" }}>{alert.type}</span>
