  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <div style={{ flex: 1, height: 4, background: '#16213e', borderRadius: 2, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', left: `${minPct}%`, width: 4, height: 8, background: '#d4af37', borderRadius: 2, transform: 'translateX(-2px)', top: -2 }} />
      </div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: barHeight, padding: '4px 0' }}>
      {values.map((v, i) => {
        ...
        return (
          <div key={i} style={{ flex: 1, height: h, background: i >= values.length - 1 ? '#d4af37' : '#3b82f6', borderRadius: '2px 2px 0 0', opacity: 0.7 }} />
        )
      })}
    </div>
  )
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {totalLower > 0 && totalUpper > 0 && (
        <div style={{ background: '#1a1a24', borderRadius: 8, padding: 16, border: '1px solid #2a2a4a' }}>
          ...
          <div style={{ height: 6, background: '#16213e', borderRadius: 3, position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', left: '50%', width: 6, height: 12, background: '#d4af37', borderRadius: 2, transform: 'translateX(-3px)', top: -3 }} />
          </div>
          <div style={{ textAlign: 'center', color: '#d4af37', fontSize: 11, marginTop: 4 }}>Forecast: {formatCurrency(forecast.totalRevenue)}</div>
        </div>
      )}
