...
export default function FPAWorkforcePlanning({ plans }: FPAWorkforcePlanningProps) {
  ...
  const summaryItems = [
    ...
    { label: 'Total Compensation', value: formatCurrency(totalComp), color: '#d4a843' },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {plans.map((p, i) => (
            <motion.div key={p.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
              style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 1fr 1.5fr', gap: 8, padding: '12px 12px', background: '#1a1a2e', borderRadius: 6, alignItems: 'center', border: '1px solid #2a2a4a' }}>
              ...
              <span style={{ color: '#d4a843', fontSize: 13, fontWeight: 600, fontFamily: 'ui-monospace, monospace' }}>{formatCurrency(p.totalCompensation)}</span>
            </motion.div>
          ))}
        </div>
      )}
