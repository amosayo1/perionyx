...
import type { PlanningAlert } from '@/server/fpa/types'
...
const typeColors: Record<string, string> = {
  ...
  variance: '#f97316', workforce: '#14b8a6', capital: '#22c55e',
  cash: '#22c55e', compliance: '#ef4444',
}
const severityOrder = ['emergency', 'critical', 'warning', 'info']

export interface FPAAlertsPanelProps {
  alerts: PlanningAlert[]
}

export function FPAAlertsPanel({ alerts }: FPAAlertsPanelProps) {
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    {!alert.isRead && <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#d4a843' }} />}
                    ...
                    <span style={{ background: (typeColors[alert.type] ?? '#64748b') + '22', color: typeColors[alert.type] ?? '#64748b', fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 10, textTransform: 'capitalize' }}>{alert.type}</span>
