...
import type { PlanningRecommendation } from '@/server/fpa/types'
...
const typeIcons: Record<string, string> = {
  ...
  cost: '\u2699', revenue: '\ud83d\udcc8', workforce: '\ud83d\udc65',
  capital: '\ud83c\udfdb\ufe0f', strategic: '\ud83c\udfaf',
}
const priorityOrder = ['critical', 'high', 'medium', 'low']

export interface FPARecommendationsPanelProps {
  recommendations: PlanningRecommendation[]
}

export function FPARecommendationsPanel({ recommendations }: FPARecommendationsPanelProps) {
            ...
            <div key={priority} style={{ marginBottom: 20 }}>
              ...
              {items.map((r, i) => (
                <motion.div key={r.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ delay: i * 0.03 }}
                  ...
                  {r.estimatedSavings != null && r.estimatedSavings > 0 && (
                    <div style={{ fontSize: 11, color: '#22c55e', marginBottom: 8 }}>
                      Est. savings: {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 }).format(r.estimatedSavings)}
                    </div>
                  )}
