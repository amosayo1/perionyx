...
const SEVERITY_CONFIG: Record<string, { icon: React.ElementType; color: string; border: string; bg: string; dot: string }> = {
...
export const AlertsPanel = memo(function AlertsPanel({ alerts, onDismiss, className }: AlertsPanelProps) {
  ...
  return (
    <div className={cn("space-y-2", className)}>
      {sorted.map((alert) => {
        ...
        return (
          <div key={alert.id} className={cn("rounded-lg border bg-zinc-900/40 transition-colors hover:bg-zinc-900/60", config.border)}>
            <div className="flex items-start gap-3 px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className={cn("text-sm font-medium text-white", alert.severity === "critical" && "text-red-300")}>
                        {alert.title}
                      </span>
                ...
                {!isExpanded && <p className="mt-1 truncate text-xs text-zinc-500">{alert.message}</p>}
