...
export function IntegrationKpiCard({ kpi }: { kpi: IntegrationKpi }) {
  ...
  const trendColor = trendUp ? "text-[#d4af37]" : trendDown ? "text-red-400" : "text-zinc-500";
    ...
    >
      <div className="flex items-start justify-between mb-2">
        ...
        <div className={cn("flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-[10px] font-medium", statusColors[kpi.status] ?? statusColors.inactive)}>
          <span className={cn("h-1.5 w-1.5 rounded-full", kpi.status === "healthy" && "bg-[#d4af37]", kpi.status === "warning" && "bg-amber-500", kpi.status === "critical" && "bg-red-500", kpi.status === "inactive" && "bg-zinc-500")} />
