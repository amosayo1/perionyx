...
export function ComplianceScoreChart() {
  const data = useMemo(() => MOCK_TREND_DATA.complianceScore, [])
  const maxVal = Math.max(...data.map((d) => d.value), 1)

  return (
    <div className="rounded-lg border border-white/[0.06] bg-zinc-900/50 p-5" role="region" aria-label="Compliance score trend">
        ...
        {data.map((point) => {
          ...
          return (
            <div key={point.date} className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end">
              <span className="text-[10px] text-emerald-400 font-medium tabular-nums">{point.value}%</span>
              <div className="w-full rounded-t overflow-hidden flex flex-col justify-end" style={{ height: `${pct}%` }}>
                <div
                  className="w-full rounded-t bg-gradient-to-t from-emerald-700 to-emerald-400 transition-all"
                  style={{ height: "100%" }}
