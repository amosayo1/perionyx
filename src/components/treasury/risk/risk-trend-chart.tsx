...
export function RiskTrendChart({ className }: { className?: string }) {
  const data = MOCK_TREND_DATA.riskTrend;
  const maxVal = 100;

  return (
    <div className={cn("rounded-lg border border-white/[0.06] bg-zinc-900/50 p-5", className)} dir="ltr">
      <h3 className="text-sm font-medium text-white">Risk Trend</h3>
      <p className="mb-4 text-[12px] text-zinc-500">Overall risk score by month</p>
