...
export function CashBurnChart({ className }: { className?: string }) {
  const data = MOCK_TREND_DATA.cashBurn
  const maxVal = Math.max(...data.map((d) => d.value), 1)

  return (
    <div className={cn("rounded-lg border border-white/[0.06] bg-zinc-900/50 p-5", className)}>
      <h3 className="text-sm font-medium text-white">Cash Burn</h3>
      <p className="mt-0.5 text-xs text-zinc-400">Monthly cash consumption</p>
      <div className="mt-4" dir="ltr" aria-label="Cash burn chart">
            ...
            return (
              <div key={i} className="flex flex-1 flex-col items-center gap-1">
                ...
                <div
                  ...
                  style={{ height: barH }}
                  role="img"
                  aria-label={`${point.label}: ${formatCash(point.value)} burn`}
                />
