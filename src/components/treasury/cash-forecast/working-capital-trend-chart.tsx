...
export function WorkingCapitalTrendChart({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("rounded-lg border border-white/[0.06] bg-zinc-900/50 p-5", className)}>
      <h3 className="text-sm font-medium text-white">Working Capital</h3>
      <p className="mt-0.5 text-xs text-zinc-400">Trend over time</p>
      <div className="mt-4" dir="ltr" aria-label="Working capital trend chart">
          ...
          {data.map((point, i) => {
            ...
            return (
              <div key={i} className="flex flex-1 flex-col items-center gap-1">
                ...
                <div
                  className="w-full rounded-sm bg-gradient-to-t from-cyan-700 to-cyan-500"
                  style={{ height: barH }}
                  role="img"
