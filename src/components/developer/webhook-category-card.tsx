...
const statusConfig: Record<string, { label: string; dot: string }> = {
  healthy: { label: "Healthy", dot: "bg-[#d4af37]" },
    ...
    <div className="rounded-xl border border-white/[0.06] bg-zinc-900/40 p-3 transition-all hover:bg-zinc-900/60">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className={cn("text-[10px] font-semibold", category.deliveryStatus === "healthy" && "text-[#d4af37]", category.deliveryStatus === "warning" && "text-amber-400", category.deliveryStatus === "degraded" && "text-red-400")}>
            {cfg.label}
          </span>
        </div>
      </div>
