...
export function PlatformHealthCard({ metric }: { metric: PlatformHealthMetric }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 mt-1">
        ...
        <span className={cn("text-[11px] font-medium", trendColors[metric.trend])}>
          {metric.trendLabel}
        </span>
      </div>
      ...
      <div className="flex items-end gap-[2px] h-6 mt-2">
        {metric.sparklineData.map((point, i) => {
          ...
          const h = Math.max((point / max) * 22, 3);
          return (
            <div
              key={i}
              className={cn(
