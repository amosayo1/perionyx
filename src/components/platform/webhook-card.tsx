...
export function WebhookCard({ webhook }: { webhook: WebhookEndpoint }) {
  ...
  const rateColor = webhook.successRate >= 99 ? "text-gold" : webhook.successRate >= 97 ? "text-amber-400" : "text-red-400";
  ...
  return (
    <div className="rounded-xl border border-white/[0.06] bg-zinc-900/40 p-3">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className={cn("text-[10px] font-semibold", webhook.status === "connected" && "text-gold", webhook.status === "disconnected" && "text-red-400", (webhook.status === "warning" || webhook.status === "retrying") && "text-amber-400")}>
          {cfg.label}
        </span>
      </div>
